        ...
        ${items.map((item: any) => `
          <tr>
            <td>
              ...
              ${item.adult_quantity ? `<div class="item-description">Adults: ${item.adult_quantity}</div>` : ''}
              ${item.child_quantity ? `<div class="item-description">Children: ${item.child_quantity}</div>` : ''}
            </td>
            <td style="text-align: center;">${item.quantity || 1}</td>
            <td style="text-align: right;">${formatCurrency(item.unit_price, order.currency)}</td>
            <td style="text-align: right;">${formatCurrency(item.subtotal, order.currency)}</td>
          </tr>
        `).join('')}
      </tbody>
    </table>
