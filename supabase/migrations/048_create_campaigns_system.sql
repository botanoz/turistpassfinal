...
-- =============================================
-- This migration creates a comprehensive campaign management system
-- for banners, discount codes, and promotions

-- =============================================
-- 1. CAMPAIGNS TABLE (Main promotional campaigns)
-- =============================================
CREATE TABLE IF NOT EXISTS campaigns (
  ...
  -- Display Settings
  banner_text TEXT NOT NULL, -- Text shown in banner
  ...
  -- Discount Settings (for display purposes)
  ...
  -- Targeting (optional JSON for future targeting features)
  target_audience JSONB DEFAULT '{}', -- e.g., {"user_type": "new", "location": "istanbul"}
...
CREATE TABLE IF NOT EXISTS discount_codes (
  ...
  -- Campaign Association (optional - codes can exist without campaigns)
  campaign_id UUID REFERENCES campaigns(id) ON DELETE SET NULL,

  -- Code Details
  code TEXT UNIQUE NOT NULL, -- e.g., "SUMMER15", "WELCOME20"
  description TEXT,
  ...
  -- Applicable Passes (NULL = all passes)
  applicable_pass_ids UUID[] DEFAULT NULL, -- Array of pass IDs
  applicable_pass_types TEXT[] DEFAULT NULL, -- Array of pass types
...
-- Anonymous users can view active campaigns
...
-- Customers can view active codes (for validation)
...
-- Anonymous can view active codes (for validation)
...
INSERT INTO campaigns (
  ...
  'Save 15% on all passes',
  'Limited time offer! Get 15% off on all TuristPass packages. Use code SUMMER15 at checkout. Valid until end of summer season.',
  'Limited Time Offer: 15% OFF Summer Promotion - Don''t Miss Out!',
  'promotion',
  'percentage',
  15,
  NOW(),
...
-- Insert corresponding discount code
...
SELECT
  ...
  'active'
FROM campaigns c
WHERE c.title = 'Summer Promotion 2025'
...
COMMENT ON TABLE campaigns IS 'Promotional campaigns with banner display and discount information';
COMMENT ON TABLE discount_codes IS 'Discount/coupon codes that can be applied during checkout';
COMMENT ON TABLE discount_code_usage IS 'History of discount code usage by customers';

COMMENT ON COLUMN campaigns.priority IS 'Higher priority campaigns are displayed first (when multiple active)';
COMMENT ON COLUMN campaigns.target_audience IS 'JSON field for future audience targeting features';

COMMENT ON COLUMN discount_codes.max_uses IS 'Maximum total uses for this code. NULL = unlimited';
COMMENT ON COLUMN discount_codes.max_uses_per_customer IS 'Maximum uses per customer. Default is 1';
COMMENT ON COLUMN discount_codes.applicable_pass_ids IS 'Array of pass IDs this code applies to. NULL = all passes';

COMMENT ON FUNCTION get_active_banner_campaigns() IS 'Returns the highest priority active campaign for banner display';
COMMENT ON FUNCTION validate_discount_code(TEXT, UUID, NUMERIC, UUID) IS 'Validates a discount code and returns discount details or error message';
